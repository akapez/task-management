...
const LeftNavigation: FC = () => {
  ...
  return (
    <>
      <nav className="fixed bottom-0 left-0 top-16 z-30 w-60 shrink-0 border-r border-gray-200 bg-white shadow-md">
            ...
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className={cn(
                    "m-3 flex items-center rounded-md bg-gray-100 px-4 py-2 font-semibold text-dark-400 hover:bg-gray-300",
                    pathname === item.href &&
